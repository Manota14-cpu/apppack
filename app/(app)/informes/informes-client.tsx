...
import { Info, PackageSearch, Receipt, TrendingUp, TriangleAlert } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from "@/components/ui/select";
import { PERIODOS, etiquetaPeriodo, type Informe } from "@/lib/informes";
import { cuentaEnResultado, etiquetaCategoria } from "@/lib/gastos";
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <Link href="/gastos" className="text-caption text-muted-foreground underline">
            Ver todos
          </Link>
        </div>

        {informe.gastos.cantidad === 0 ? (
          <p className="rounded-xl border border-border p-6 text-center text-caption text-muted-foreground">
            No hay gastos anotados en el período. Hasta que los cargues, el margen de arriba es
            solo la diferencia entre el precio y el costo de la mercadería: no descuenta el flete,
            la luz ni el alquiler.{" "}
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-3">
              <Resumen
                titulo="Resultado"
                valor={informe.resultado !== null ? money(informe.resultado) : "—"}
                detalle={
              ...
              <Resumen
                ...
                valor={money(informe.gastos.operativos)}
                detalle={`${num(informe.gastos.cantidad)} ${informe.gastos.cantidad === 1 ? "gasto anotado" : "gastos anotados"}`}
              />
              <Resumen
                titulo="Mercadería comprada"
                valor={money(informe.gastos.mercaderia)}
                detalle="No se resta: es stock"
              />
            </div>

            <ul className="divide-y divide-border rounded-xl border border-border">
              {informe.gastos.porCategoria.map((g) => (
                <li key={g.categoria} className="flex flex-wrap items-center gap-x-4 gap-y-1 px-3 py-2.5">
                  <span className="min-w-0 flex-1 text-caption font-medium">
                    {etiquetaCategoria(g.categoria)}
                  </span>
                  <span className="text-caption text-muted-foreground">
                    {num(g.cantidad)} {g.cantidad === 1 ? "gasto" : "gastos"}
                  </span>
                  {!cuentaEnResultado(g.categoria) && <Badge variant="outline">No resta</Badge>}
                  <span className="font-mono-num text-caption font-semibold">{money(g.total)}</span>
                </li>
              ))}
            </ul>

            {/* La trampa más fácil de este informe: restar dos veces la
                mercadería y creer que el mes fue malo. */}
            {informe.gastos.mercaderia > 0 && (
              <p className="text-caption text-muted-foreground">
                Lo que se le pagó a los proveedores por mercadería no se resta del resultado: son
                pesos que se cambiaron por stock. Se cuentan como costo recién cuando ese stock se
                vende, y ahí ya están adentro del margen de arriba.
              </p>
            )}
          </>
        )}
